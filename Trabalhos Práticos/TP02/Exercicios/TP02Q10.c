...
bool ordenacao(jogador p1, jogador p2){
    ...
    if(isNaoInf(n1)){
        if(isNaoInf(n2)){
            ...
            if(n1[i]!='\0' && n2[i] != '\0' && n1[i] > n2[i]) result = true;
            return result;
        }else{
            return true;
        }
...
void quicksort(jogador ** lista, int esq, int dir){
    int i = esq;
    int j = dir;
    jogador pivo = (*lista)[(esq+dir)/2];
    while(i <= j){
            while(ordenacao(pivo,(*lista)[i] )){
                i++;
            }
            while(ordenacao((*lista)[j] ,pivo) ){
                j--;
            }
            if(i<=j){
                swap(lista,i,j);
...
void quicksort2(jogador ** lista, int tam){
    int esq = 0;
    int dir = tam;
    quicksort(lista,esq,dir);
}
...
int main(){
    ...
    FILE* arq = fopen("801792_quicksort.txt", "w");
